...
function HangmanGame({day,seed,isToday,archiveNav,chipBar,onHome,onArchive}){
  ...
  if(savedToday)return(<div style={T.app}><Hdr title="Impiccato" sub={`🗓 Giornaliero · #${day}`} onHome={onHome}/><DoneScreen gameKey="hangman" day={day} isToday={isToday} onHome={onHome} onArchive={onArchive}>{(s)=><>
    ...
    <div style={{display:"flex",gap:"8px",justifyContent:"center",marginBottom:"12px"}}>
      <div style={{background:US.surface,border:`1px solid ${US.border}`,borderRadius:"8px",padding:"8px 14px",textAlign:"center"}}>
        <div style={{fontSize:"18px",fontWeight:"700",color:s.won?US.green:US.red}}>{s.errors||0}</div>
        <div style={{fontSize:"9px",color:US.muted,textTransform:"uppercase",letterSpacing:"1px"}}>errori</div>
      </div>
      <div style={{background:US.surface,border:`1px solid ${US.border}`,borderRadius:"8px",padding:"8px 14px",textAlign:"center"}}>
        <div style={{fontSize:"18px",fontWeight:"700",color:US.black}}>{s.word?s.word.length:0}</div>
        <div style={{fontSize:"9px",color:US.muted,textTransform:"uppercase",letterSpacing:"1px"}}>lettere</div>
      </div>
    </div>
...
function HigherOrLowerGame({onHome}){
  ...
  function rndPlayer(exclude){
    let p,tries=0;
    do{p=DB_HL[Math.floor(Math.random()*DB_HL.length)];tries++;}
    while(exclude&&p.name===exclude.name&&tries<50);
    return p;
  }

  const[base,setBase]=useState(()=>rndPlayer(null));
  const[challenger,setChallenger]=useState(()=>{const b=rndPlayer(null);return rndPlayer(b);});
  const[chosen,setChosen]=useState(null); // 'base'|'challenger'
  ...
  function choose(who){
    ...
    setTimeout(()=>{
      if(ok){
        ...
        setStreak(ns);
        setRound(r=>r+1);
        if(ns>best){setBest(ns);try{localStorage.setItem('hl_best',String(ns));}catch{}}
        if(ns>0&&(ns%5===0||ns===1))setConfetti(true);
        setTimeout(()=>setConfetti(false),2500);
        // Challenger (ora noto) va a sinistra, nuovo sconosciuto a destra
        const newChallenger=rndPlayer(challenger);
        setBase(challenger);
  ...
  if(done)return(
    <div style={T.app}>
      ...
      <Hdr title="Higher or Lower" onHome={onHome}/>
      <div style={{...T.body,textAlign:"center",paddingTop:"28px"}} className="pop-in">
        ...
        <div style={{background:US.surface,borderRadius:"10px",padding:"14px",marginBottom:"16px",border:`1px solid ${US.border}`,textAlign:"left"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
            {[{p:base,label:"Base"},{p:challenger,label:"Sfidante"}].map(({p,label})=>(
              <div key={p.name} style={{background:p.name===winner==='base'?base.name:challenger.name===p.name&&winner==='challenger'?"#f0fdf4":"#fef2f2",borderRadius:"8px",padding:"10px",textAlign:"center",border:`1.5px solid ${p===base&&winner==='base'||p===challenger&&winner==='challenger'?"#16a34a":"#dc2626"}`}}>
                ...
                <div style={{fontSize:"10px",color:US.muted}}>{p.club}</div>
                <div style={{fontSize:"18px",fontWeight:"800",color:"#333",marginTop:"6px"}}>€{p.value}M</div>
              </div>
            ))}
          </div>
        </div>
  ...
  return(
    <div style={T.app}>
      {confetti&&<Confetti active={confetti}/>}
      <Hdr title="Higher or Lower" sub={`💶 Confronto #${round}`} onHome={onHome}/>
      <div style={{...T.body,maxWidth:"460px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"16px"}}>
          ...
          <div style={{fontSize:"12px",color:US.muted,fontWeight:"600"}}>Chi vale di più?</div>
        </div>

        {/* Cards */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
          {[
            {p:base,label:"",showVal:true,who:"base"},
            {p:challenger,label:"",showVal:!!chosen,who:"challenger"}
          ].map(({p,label,showVal,who})=>{
        ...
        {!chosen&&<div style={{textAlign:"center",fontSize:"11px",color:US.muted}}>
          Clicca sul giocatore che vale di più
        </div>}
      </div>
    </div>
  );
...
function CarreiraGame({day,seed,isToday,archiveNav,chipBar,onHome,onArchive}){
  ...
  return(<div style={T.app}><Hdr title="Indovina dalla Carriera" sub={`${label} · #${day}`} onHome={onHome} archiveNav={archiveNav}/>{chipBar||null}
    <div style={{...T.body,maxWidth:"520px"}}>
      ...
      {st==="c"&&<><div style={{padding:"8px",background:US.greenL,border:"1px solid #bbf7d0",borderRadius:"2px",color:US.green,fontSize:"12px",marginBottom:"9px"}}>✓ Corretto! Era <strong>{player.answer}</strong> — <strong>+{pts} punti</strong></div><div style={{textAlign:"right"}}><button onClick={()=>{if(isToday)saveResult("carriera",{score:sc,done:true});sFin(true);setTimeout(()=>setCarConf(true),400);}} style={T.pb}>Risultato →</button></div></>}
...
function RosaQuizGame({day,seed,isToday,archiveNav,chipBar,onHome,onArchive}){
  ...
  const BONUS=5;
  const[input,setInput]=useState("");const[found,setFound]=useState([]);const[wrong,setWrong]=useState(null);const[seconds,setSeconds]=useState(TOTAL);const[lastFound,setLastFound]=useState(null);const[done,setDone]=useState(false);const[bonusFlash,setBonusFlash]=useState(false);const[rosaConf,setRosaConf]=useState(false);
  ...
  useEffect(()=>{
    ...
    timerRef.current=setInterval(()=>setSeconds(s=>{if(s<=1){clearInterval(timerRef.current);setDone(true);setTimeout(()=>setRosaConf(true),300);return 0;}return s-1;}),1000);
  ...
  if(savedToday&&isToday)return(<div style={{...T.app,position:"relative"}}>{rosaConf&&<Confetti active={rosaConf}/>}<Hdr title="Indovina la Rosa" sub="🗓 Giornaliero" onHome={onHome}/><DoneScreen gameKey="rosa" day={day} isToday={isToday} onHome={onHome} onArchive={onArchive}>{(s)=>{
  ...
  return(<div style={T.app}><Hdr title={`Rosa Quiz · ${squadra.emoji} ${squadra.nome}`} sub={`${label} · #${day}`} onHome={onHome} archiveNav={archiveNav}/>{chipBar||null}
    <div style={T.body}>
      ...
      <div style={{marginBottom:"12px"}}>
        ...
        {wrong&&<div style={{fontSize:"12px",color:US.red,marginTop:"4px"}}>✗ "{wrong}" — non trovato</div>}
      </div>
...
function TransferGame({day,seed,isToday,archiveNav,chipBar,onHome,onArchive}){
  ...
  const[trConf,setTrConf]=useState(false);
  ...
  function submitField(){
    ...
    setCurInput("");
    if(step<2)setStep(s=>s+1);else{const finalRes={...results,[FIELDS[step].key]:evalField(FIELDS[step].key,curInput)};if(isToday)saveResult("transfer",{score:Object.values(finalRes).filter(v=>v==="green").length,player:tr.player,to:tr.to});setStep(3);setTimeout(()=>setTrConf(true),400);}
  }
  ...
  return(<div style={{...T.app,position:"relative"}}>{trConf&&<Confetti active={trConf}/>}<Hdr title="Indovina il Trasferimento" sub={`${label} · #${day}`} onHome={onHome} archiveNav={archiveNav}/>{chipBar||null}
    <div style={{...T.body,maxWidth:"480px"}}>
      ...
      {step===3&&<div className="pop-in" style={{textAlign:"center",padding:"12px",background:score===3?US.greenL:score>=1?"#fffbea":US.redL,borderRadius:"6px",color:score===3?US.green:score>=1?US.yellow:US.red,marginTop:"4px"}}>
...
function ConnectionsGame({day,seed,isToday,archiveNav,chipBar,onHome,onArchive}){
  ...
  if(savedToday)return(
    ...
    <DoneScreen gameKey="connections" day={day} isToday={isToday} onHome={onHome} onArchive={onArchive}>{(s)=><>
      ...
      <div style={{fontSize:"12px",color:US.muted,marginBottom:"10px"}}>Errori: {s.errors}/{MAX_ERRORS}</div>
      <div style={{marginBottom:"10px"}}>
        {puzzle.groups.map(g=>(
          <div key={g.label} style={{background:CONN_COLORS[g.color].bg,borderRadius:"6px",padding:"7px 10px",marginBottom:"5px",textAlign:"center"}}>
            <div style={{fontSize:"10px",fontWeight:"700",color:"#fff",textTransform:"uppercase",marginBottom:"2px"}}>{g.label}</div>
            <div style={{fontSize:"10px",color:"rgba(255,255,255,0.85)"}}>{g.players.join(", ")}</div>
          </div>
        ))}
      </div>
  ...
  return(<div style={{...T.app,position:"relative"}}>
    ...
    <div style={{...T.body,maxWidth:"520px"}}>
      ...
      {/* Gruppi risolti */}
      {solved.map(g=>(
        <div key={g.label} className="conn-solve" style={{background:CONN_COLORS[g.color].bg,borderRadius:"6px",padding:"10px 14px",marginBottom:"6px",textAlign:"center"}}>
      ...
      {done&&<div className="pop-in" style={{textAlign:"center",marginTop:"8px"}}>
        ...
        {!isToday&&archiveNav&&archiveNav.day<archiveNav.max&&<button onClick={archiveNav.next} style={{...T.pb,width:"100%",marginBottom:"6px"}}>→ Prossima sfida</button>}{isToday&&onArchive&&<button onClick={onArchive} style={{...T.sb,width:"100%",marginTop:"6px",color:US.black}}>📂 Archivio</button>}
  ...
  return(<div style={{...T.app,position:'relative'}}>
    ...
    <div style={T.body}>
      ...
      {won&&<div style={{textAlign:'center',padding:'14px',background:'#f0fdf4',
        ...
        {!isToday&&archiveNav&&archiveNav.day<archiveNav.max&&<button onClick={archiveNav.next} style={{...T.pb,width:'100%',marginBottom:'6px'}}>→ Prossima sfida</button>}
        <button onClick={onHome} style={{...T.pb,marginTop:'8px',width:'100%'}}>Home</button>
      </div>}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'5px'}}>
        {guesses.map(({p,rank,isWin},idx)=>{
          ...
          const prevRank=idx<guesses.length-1?guesses[idx+1].rank:null;
          const arrow=isWin?'':prevRank===null?'':rank<prevRank?'↑':rank>prevRank?'↓':'=';
          const arrowCol=arrow==='↑'?'#22c55e':arrow==='↓'?'#ef4444':'#888';
          return(
            <div key={idx} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:'8px',
              ...
              <div>
                <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                  <span style={{fontSize:'13px',fontWeight:'500',color:US.black}}>{p.n}</span>
...
function Card({m,onDaily,onArchive}){
  ...
  return(<div style={{background:"#fff",border:`1px solid ${borderColor}`,...accentStyle,borderRadius:"8px",padding:"11px",transition:"border-color 0.15s",display:"flex",flexDirection:"column",justifyContent:"space-between",minHeight:isBig?"130px":"110px"}} onMouseEnter={()=>sHv(true)} onMouseLeave={()=>sHv(false)}>
    ...
    <div style={{display:"flex",gap:"3px",marginTop:"8px"}}>
      <button onClick={()=>onDaily(m.key)} style={{flex:1,background:US.orange,color:US.black,border:"none",borderRadius:"4px",padding:"8px 2px",fontSize:"9px",fontWeight:"700",textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.3px",minHeight:"36px"}}>🗓 Daily</button>
      <button onClick={()=>onArchive(m.key)} style={{flex:1,background:US.black,color:"#fff",border:"none",borderRadius:"4px",padding:"8px 2px",fontSize:"9px",fontWeight:"700",textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.3px",minHeight:"36px"}}>📂 Arch.</button>
    </div>
  </div>);
}
...
function Home({onSelect}){
  ...
  return(<div style={{...T.app,paddingBottom:"40px"}}>
    <div style={{background:US.black,color:"#fff",padding:"18px 18px 16px",borderBottom:`3px solid ${US.orange}`}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
        <div style={{fontSize:"22px",fontWeight:"700",letterSpacing:"-0.5px",lineHeight:1}}>Quiz Calcio</div>
      ...
      <div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"5px"}}>
          ...
          <div style={{fontSize:"9px",color:playedToday===10?US.green:"#888",fontWeight:"700"}}>
            {playedToday}/10 {playedToday===10?"✓ completate":"completate"}
          </div>
        </div>
        <div style={{height:"4px",background:"#1a1a1a",borderRadius:"2px",overflow:"hidden"}}>
          <div style={{height:"100%",width:`${playedToday/10*100}%`,background:playedToday===10?US.green:US.orange,
    ...
    <div style={{padding:"12px 12px 40px",maxWidth:"620px",margin:"0 auto",boxSizing:"border-box"}}>
      ...
      <div style={{background:US.black,borderRadius:"10px",padding:"14px 16px",marginBottom:"10px",
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
          <div>
            <div style={{fontSize:"24px",fontWeight:"700",color:US.orange,lineHeight:1}}>2.508</div>
            <div style={{fontSize:"9px",color:"#666",textTransform:"uppercase",letterSpacing:"1.5px",marginTop:"2px"}}>sfide disponibili</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:"11px",color:"#fff",fontWeight:"700",marginBottom:"4px"}}>10 modalità di gioco</div>
  ...
  useEffect(()=>{
    ...
    s.innerHTML=`
      *{-webkit-tap-highlight-color:transparent;} button:active{opacity:0.75;transform:scale(0.97);}
      ...
      .conn-solve{animation:slideDown 0.5s ease forwards;}
